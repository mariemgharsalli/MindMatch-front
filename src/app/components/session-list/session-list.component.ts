...
export class SessionListComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        if (this.sessionForm.value.location) {
          const [lat, lng] = this.sessionForm.value.location
            .split(',')
            .map(Number);
          if (!isNaN(lat) && !isNaN(lng)) {
            this.updateMarkerPosition({ lat, lng });
  ...
  showArchiveNotification() {
    this.notificationMessage =
      'Cette session sera automatiquement supprimée après 1 heure.';
  ...
  unarchiveSession(id: number): void {
    this.sessionService.unarchiveSession(id).subscribe({
      ...
      error: (err) => {
        ...
        this.notificationMessage =
          'Erreur lors de la désarchivage de la session';
        this.notificationType = 'error';
        this.showNotification = true;
        setTimeout(() => this.hideNotification(), 5000);
      },
  ...
  searchSession(): void {
    if (this.searchQuery.trim()) {
      const targetArray =
        this.activeTab === 'active' ? this.sessions : this.archivedSessions;
      const filtered = targetArray.filter(
        (session) =>
