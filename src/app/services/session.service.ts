...
export class SessionService {
  ...
  uploadUserImages(formData: FormData, sessionId: any) {
    return this.http.post<Session>(
      `${this.baseUrl}/images/upload/${sessionId}`,
      formData
    );
  }
  ...
  archiveSession(id: number): Observable<string> {
    return this.http
      .post(`${this.baseUrl}/archive/${id}`, null, { responseType: 'text' })
      .pipe(tap(() => this.sessionsUpdated.next()));
  }

  unarchiveSession(id: number): Observable<string> {
    return this.http
      .put(`${this.baseUrl}/unarchive/${id}`, null, { responseType: 'text' })
      .pipe(tap(() => this.sessionsUpdated.next()));
  }
